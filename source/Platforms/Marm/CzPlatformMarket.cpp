	...
	switch (market->getProductInformation()->m_ProductStoreStatus)
	{
		case S3E_PRODUCT_STORE_STATUS_VALID:
		{
			...
			if (s3eIOSAppStoreBillingGetInt(S3E_IOSAPPSTOREBILLING_CAN_MAKE_PAYMENTS) != 0)
			{
				...
				if (market->getActiveMarket() != NULL)
					market->getActiveMarket()->NotifyBillingDisabled();
			}
			else
			{
				if (market->getActiveMarket() != NULL)
					market->getActiveMarket()->NotifyInfoAvailable();
			}
			break;
		}
...
{
	s3eAndroidMarketBillingResult res = s3eAndroidMarketBillingIsSupported();
	if (res == S3E_ANDROIDMARKETBILLING_UNAVAILABLE)
		return false;
...
{
	getActiveMarket()->setCurrentProductID(product_id);
	if (getActiveMarket() != NULL)
		getActiveMarket()->NotifyError();
...
CzDebug::Log(CZ_DEBUG_CHANNEL_INFO, "******** AsyncResponse - ", CzString((int)result).c_str());
	switch (result)
	{
...
CzDebug::Log(CZ_DEBUG_CHANNEL_INFO, "******** OrderInformationChanged");
	...
	if (PurchaseTimer.hasStarted() && PurchaseTimer.HasTimedOut())
	{
		...
		if (SimulationHash != 0)
		{
			if (SimulationHash == CzHashes::Error_Hash)
				ActiveMarket->NotifyError();				// Simulate an error during purchasing
			else
			if (SimulationHash == CzHashes::Refund_Hash)
				ActiveMarket->NotifyRefund();				// Simulate a refund
			else
			if (SimulationHash == CzHashes::BillingDisabled_Hash)
				ActiveMarket->NotifyBillingDisabled();	// Simulate billing disabled
			else
			if (SimulationHash == CzHashes::Complete_Hash)
				ActiveMarket->NotifyComplete();	// Simulate successful purchase
		}
		else
		{
...
bool CzPlatformMarketTest::QueryProduct(const char* product_id)
{
	...
	ActiveMarket->NotifyInfoAvailable();	// Simulate info available
...
void CzPlatformMarketTest::ForceError(eMarketStatus status)
{
	setStatus(status);
	if (ActiveMarket != NULL)
		ActiveMarket->NotifyError();	// Simulate error
}
